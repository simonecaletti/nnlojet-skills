...
Spec fields (see example_spec.json):
  ...
  mode           : "soft" | "collinear"
  limit          : PREFERRED: the shared mode vocabulary (irlimits.py,
                   same families as genuine_modes.py), e.g.
                   {"family": "sc", "unresolved": [ISOFT, IA, IB],
                    "spectators": [...]} — needs "npar" (kin-set index).
  em_expr        : escape hatch (with limit_call) when "limit" is not
                   given: e.g. "sqrts_proc*dsqrt(1d0-xs)" (soft)
  ...
  cuts_call      : verbatim, e.g. "call ecuts_epem(1,7,ipass)";
                   optional (a bare antenna target needs no cuts)
...
def generate(spec):
    ...
    # The limit may be named in the shared mode vocabulary (irlimits:
    # ss/sco/ds/tc/sc/dc + indices, same as genuine_modes.py); the
    # verbatim em_expr/limit_call pair stays as the escape hatch.
    if "limit" in spec:
        from irlimits import from_spec as limit_from_spec
        llines, _, _ = limit_from_spec(spec["limit"], spec["npar"])
        limit_block = "\n".join("          " + ln for ln in llines)
    else:
        limit_block = ("          em1=" + spec["em_expr"] + "\n"
                       + "          " + spec["limit_call"])
    ...
    out += LOOP_HEAD.format(xsdata=xsdata, npt=spec["npt"],
                            ...
                            cuts_call=spec.get("cuts_call", "continue"),
                            full_me=target)
...
def selftest():
    ...
    # shared mode vocabulary replaces em_expr/limit_call
    voc = dict(coll, npar=5)
    voc.pop("em_expr")
    voc.pop("limit_call")
    voc["limit"] = {"family": "sco", "unresolved": [3, 4],
                    "spectators": [5]}
    src = generate(voc)
    assert "call get_sco5(sqrts_proc,3,4,em1,5)" in src
    assert "em1=sqrts_proc*xs" in src
    # cuts_call optional
    voc.pop("cuts_call")
    src = generate(voc)
