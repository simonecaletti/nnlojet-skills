...
`[[incoming], FullME, [reduced MEs]]`, e.g. `FLAVlistepem.map` for
C1g0Z: `[C0g0Z, f1B1g0Z, f1B2g0Z, f2B1g0Z, f2B2g0Z]`. Read the
structure off it before writing any line: both `f1B*` AND `f2B*`
present ⇒ the term needs two flavour sectors; `B2g0Z` (not `Bt2g0Z`) ⇒
leading-colour reduction (the neighbouring Ct1g0Z entry lists
`f1Bt*/f2Bt*`). Honest scope: the set fixes WHICH reduced MEs appear —
not how many lines, not the colour ordering, not the argument order;
those still come from the physics and an analogous `.map`.
...
(e.g. for C1g0ZepemS: `DIS/qC1g0ZDISS.map` — one leg crossed, its
`#primary line unresolved` block maps ~1:1 onto the all-final f1
sector under a relabelling; `WJ/qqbC1g0WS.map`,
`ZJJ/qqbC1g0ZSNLO.map` — two legs crossed).
...
2. **Iterated-counterterm cluster ordering** is not crossing-invariant:
   DIS `E30FF([1],k,[j,i])` un-crosses to `E30FF([l,k],[i,k],j)` —
   clustered momentum FIRST, mirroring the X40b quark cluster. The
   transcribed order leaves a 1.2–3.4× residual visible only in a
   couple of single-collinear modes.
