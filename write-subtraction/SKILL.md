...
tests; the seven distinct repair edges hiding inside "fix it and test
again", each with its signature and which node it returns to; the
cross-channel edges; and which nodes may be worked in parallel. Load it
when implementing more than one channel, when deciding how to split the
work, or when a failure is being routed by habit rather than by its
signature.)
...
**"Fix it and test again" is seven different edges, not one** — routed by
signature, landing on four different nodes, with cost ratios around
100:1 between the cheapest and the dearest. Two of them (a stale build;
outliers that shrink with `n_azim`) need no `.map` edit at all, and one
of them unwinds all the way back to the `S,a` decomposition. The routing
table is `references/workflow-graph.md`; read it before assuming the
repair belongs in the block currently in front of you.
